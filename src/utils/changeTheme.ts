...
export default function changeTheme(v: string, swtichHover = true) {
  ...
  res.map((item, index) => {
    document.documentElement.style.setProperty(`--zui-primary${index + 1}`, item)
  ...
  if (swtichHover) {
    document.documentElement.style.setProperty('--zui-table-hover', res[0])
    document.documentElement.style.setProperty('--zui-menu-hover', res[1])
    document.documentElement.style.setProperty('--zui-menu-active', res[0])
  }
