...
export default function switchMode(mode: Mode) {
  const primary1 = document.documentElement.style.getPropertyValue(`--zui-primary1`)
  const primary2 = document.documentElement.style.getPropertyValue(`--zui-primary2`)
  ...
  Object.keys(schema).forEach((component) => {
    ...
    Object.keys(component_color_obj).forEach((item) => {
      document.documentElement.style.setProperty(`--zui-${component}-${item}`, component_color_obj[item])
    })

    if (component === 'table' && !isDarkMode) {
      document.documentElement.style.setProperty('--zui-table-hover', primary1)
    }

    if (component === 'menu' && !isDarkMode) {
      document.documentElement.style.setProperty('--zui-menu-hover', primary2)
      document.documentElement.style.setProperty('--zui-menu-active', primary1)
    }
  })
}
